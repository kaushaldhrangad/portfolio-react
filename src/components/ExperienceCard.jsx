...
const ExperienceCard = ({ experience }) => {
  return (
    <div className="card max-w-4xl  bg-gray-300 rounded-xl py-3 px-4 justify-between realative  border overflow-hidden flex flex-col gap-3 transition-all delay-[0.1s] ease-in-out hover:-translate-y-4  max-md:p-3 max-md:gap-2 max-md:w-[250px] max-lg:overflow-x-hidden max-xl:w-[500px]">
      <div className="top flex gap-3 w-full">
        ...
        <div className="body flex flex-col w-full">
          <div  className="text-sm font-semibold max-md:text-sm">
            {experience.role}
          </div>
      ...
      <div
        className="description w-full text-xs font-normal mb-2 max-md:text-sm">
        {experience?.desc && (
            ...
            <div className="skills flex flex-wrap gap-2 pt-4">
              ...
              <div className="item-wrapper flex flex-wrap gap-2 ">
                {experience?.skills?.map((skill, index) => (
                  <div key={index} className="skill text-sm font-normal text-gray-900 max-lg:text-sm">
                    • {skill}
                  </div>
                ))}
              </div>
