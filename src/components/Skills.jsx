  ...
  return (
    <div className=" flex flex-col justify-center items-center  relative z-[1]" >
      <div className="wrapper relative flex justify-between items-center flex-col w-full max-[1100px] gap-3 ">
        <h1 className=" flex font-['Libre_Franklin'] justify-center mt-8 text-3xl text-white font-bold uppercase underline" >
          Skills
        </h1>
        <div
          key={skills.id}
          ...
          {skills.map((skill, index) => (
            <div
              key={index}
              className="skill bg-neutral-800 w-full max-w-lg border border-[#A3E635] shadow-[1px_1px_18px_0px_#A3E635]  rounded-2xl px-9 py-4 " data-aos="flip-up"  data-aos-anchor-placement="top-center" >
              <div className="skill-title text-2xl text-[#A3E635] font-semibold mb-5 items-center " >
                {skill.title}
              </div>
              <div className="skill-list flex justify-center flex-wrap gap-3 mb-5">
                {skill.skills.map((item, index) => (
                  <div
                    ...
                    <div className="text-[#ffff] skills-name font-semibold text-[1.05rem]">
                      {item.name}
                    </div>
                  </div>
                ))}
